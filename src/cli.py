...
def parse_custom_labels(ctx, param, value):
    ...
    try:
        for pair in value.split(','):
            if '=' not in pair:
                raise click.BadParameter(f"Invalid label format: {pair}. Use key=value format")
...
def main(env_prefix: str, broker_url: str, host: str, port: int, prefix: str, custom_labels: Dict[str, Any], verbose: bool):
    """
